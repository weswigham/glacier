...
export type AddDataSourceAction<S extends string, T, C> = ReduxStandardAction<"ADD_DATA_SOURCE", {type: S, metadata: T, cache: C, id: DataSourceId}>;
...
export function createAddDataSourceAction<S extends string, M, C>(type: S, metadata: M, cache: C): AddDataSourceAction<S, M, C> {
    return { type: "ADD_DATA_SOURCE", payload: { type, metadata, cache, id: (id++) as DataSourceId } };
}